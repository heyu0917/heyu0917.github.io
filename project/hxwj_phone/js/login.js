$(function(){
	...
	//性别的选择
	$('.sex-select').tap(function(){
		var imgObj = $(this).find('img');
		if(imgObj.attr('src') == 'img/sex_light.png'){
			imgObj.attr('src','img/sex_light.png');
		}else{
			imgObj.attr('src','img/sex_light.png');
			$(this).siblings().find('img').attr('src','img/sex.png');
		}
	})
	...
	$("#agree_img").tap(function(){
		if($(this).attr('src') == 'img/round_light.png'){
			$(this).attr('src','img/round2.png');
		}else{
			$(this).attr('src','img/round_light2.png');
		}
	})
	//点击注册
	...
	$("#login_password").keyup(function(){
		...
		if(reg.test(userName) == true){
			...
			if(reNum.test(this.value)){
				$(".login-btn").css({'background':'#73c640','color':'#fff'});
			}
			if(this.value == ''){
	...
	$("#register_telephone").keyup(function(){
		var reNum=/^\d*$/;		
		if(reNum.test(this.value)){
			$(".register-btn").css({'background':'#73c640','color':'#fff'});
		}
		if(this.value == ''){
